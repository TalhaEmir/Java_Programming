...
public class MultiDimensionalArraysPractice2 {
    ...
    public static void main(String[] args) {
        ...
        // deeptoString multidimensionalArray


        System.out.println("==================================");

        for (int i = 0; i < arr3D.length; i++) {// index of number of each 2 dimensional array
            int[][]each2DArray=arr3D[i];
            System.out.println(Arrays.deepToString(each2DArray));//j index number of each 1 dimensional Array
            for (int j = 0; j < each2DArray.length; j++) {
                int[]each1DArray=each2DArray[j];
                System.out.println(Arrays.toString(each1DArray));
                for (int k = 0; k < each1DArray.length; k++) {//k : index number of each elements
